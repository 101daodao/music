...
如果需要更新,可使用 方式运行npx NeteaseCloudMusicApi@版本号
...
点击 并选择你 fork 的此项目并点击Import Git Repositoryimport
点击 的 PERSONAL ACCOUNTselect
直接点Continue
PROJECT NAME自己填, 选 然后直接点 接着等部署完成即可FRAMEWORK PRESETOtherDeploy
腾讯云 serverless 部署
因 在国内访问太慢(不绑定自己的域名的情况下),在此提供腾讯云 serverless 部署方法Vercel

操作方法
fork 此项目
在腾讯云serverless应用管理页面( https://console.cloud.tencent.com/sls ),点击新建应用
顶部选择 创建方式Web 应用
选择 ,点击底部Express框架下一步按钮
输入,上传方式选择,进行GitHub授权(如已授权可跳过这一步),代码仓库选择刚刚fork的项目应用名代码仓库
启动文件填入:
#!/bin/bash
export PORT=9000
/var/lang/node16/bin/node app.js
点击,等待部署完成,点击的 里的 ,正常情况会打开文档地址,点击文档可查看接口调用效果完成资源列表API网关URL例子
注意
腾讯云 serverless 并不是免费的,前三个月有免费额度,之后收费
当前(2024-08-24), 用此法创建的话, 会关联一个"日志服务-日志主题"(创建过程中没有提醒), 此服务是计量收费的默认
可以使用代理
...
可以在 Node.js 调用
v3.31.0 后支持 Node.js 调用,导入的方法为内的文件名,返回内容包含和,为状态码,为请求返回内容,参考 文件夹下的 modulestatusbodystatusbodymodule_exampletest.js
...
由于接口做了缓存处理 ( 缓存 2 分钟,不缓存数据极容易引起网易服务器高频 ip 错误 , 可在 app.js 设置 , 可能会导致登录后获取不到 cookie), 相同的 url 会在两分钟内只向网易服务器发一次请求 , 如果遇到不需要缓 存结果的接口 , 可在请求 url 后面加一个时间戳参数使 url 不同 , 例子 : (之所以加入缓存机制是因为项目早期没有缓存机制，很多 issues 都是报 IP 高频，请按自己需求改造缓存中间件(app.js)，源码不复杂)/simi/playlist?id=347230&timestamp=1503019930000

不要频繁调登录接口,不然可能会被风控,登录状态还存在就不要重复调登录接口

如果是跨域请求 , 请在所有请求带上 (axios 为 , Fetch API 为 ), 或直接手动传入 cookie (参见 ), 否则 可能会因为没带上 cookie 导致 301, 具体例子可看 , 访问(默认端口的话) 例子使用 jQuery 和 axiosxhrFields: { withCredentials: true }withCredentials: truefetch(url, { credentials: 'include' })登录public/test.htmlhttp://localhost:3000/test.html
...
由于网易限制,此项目在国外服务器或部分国内云服务上使用会受到限制,如 ,如需解决 , 可使用参数,传进国内IP解决,如: 即可解决460 cheating异常realIP?realIP=116.25.146.177

图片加上 可控制图片尺寸，如 , ?param=宽y高http://p4.music.126.net/JzNK4a5PjjPIXAgVlqEc5Q==/109951164154280311.jpg?param=200y200http://p4.music.126.net/JzNK4a5PjjPIXAgVlqEc5Q==/109951164154280311.jpg?param=50y50

分页接口返回字段里有,more 为 true 则为有下一页more
...
登录
说明 : 登录有三个接口,建议使用对密码编码或者使用请求,避免某些特殊字符无法解析,如(在 url 中会被识别为 hash,而不是 query)encodeURIComponentPOST##
...
可选参数 :
: 国家码，用于国外手机号登录，例如美国传入：countrycode1

md5_password: md5 加密后的密码,传入后 参数将失效password

captcha: 验证码,使用 /captcha/sent接口传入手机号获取验证码,调用此接口传入验证码,可使用验证码登录,传入后 参数将失效password
...
md5_password: md5 加密后的密码,传入后 将失效password
...
v3.30.0 后支持手动传入 cookie,登录接口返回内容新增 字段,保存到本地后,get 请求带上 (先使用 编码 cookie 值) 或者 post 请求 body 带上 即可,如: 或者cookie?cookie=xxxencodeURIComponent()cookie/user/cloud?cookie=xxx

{
    ...,
    cookie:"xxx"
}
另外的cookie说明: 可以直接从浏览器中获取cookie值, 只需要其中key为的数据即可 请求MUSIC_U
...
可选参数: 传入后会额外返回二维码图片 base64 编码qrimg
...
countrycode: 国家码，用于国外手机号，例如美国传入： ,默认 86 即中国1
...
可选参数 :
: 国家码，用于国外手机号，例如美国传入： ,默认 86 即中国countrycode1
...
重复昵称检测
说明 : 调用此接口 ,可检测昵称是否重复,并提供备用昵称 必选参数 : : 昵称nickname
...
更新头像
说明 : 登录后调用此接口,使用上传图片 formData(name 为'imgFile'),可更新头像(参考: https://gitlab.com/Binaryify/NeteaseCloudMusicApi/blob/main/public/avatar_update.html ),支持命令行调用,参考module_example目录下'Content-Type': 'multipart/form-data'avatar_upload.js

可选参数 :

imgSize : 图片尺寸,默认为 300

imgX : 水平裁剪偏移,方形图片可不传,默认为 0 : 垂直裁剪偏移,方形图片可不传,默认为 0imgY
...
歌单封面上传
说明 ： 登录后调用此接口，使用上传图片 formData（name 为'imgFile'），可更新歌单封面（参考：https://gitlab.com/Binaryify/NeteaseCloudMusicApi/blob/main/public/playlist_cover_update.html'Content-Type': 'multipart/form-data')
...
调用例子 ： （需要换成自己的用户 ID）/user/comment/history?uid=32953014/user/comment/history?uid=32953014&limit=1&time=1616217577564
...
lasttime： 返回数据的 ，默认-1，传入上一次返回结果的 lasttime，将会返回下一页的数据lasttime
...
必选参数 ： ： 资源 id （歌曲，歌单，mv，电台，电台节目对应 id）id

可选参数 ： ： 资源类型，默认歌曲 song，可传 ,,,,,typesongplaylistmvdjradiodjprogramalbum

msg： 内容，140 字限制，支持 emoji，@用户名（接口获取的用户名，用户名后和内容应该有空格），图片暂不支持/user/follows
...
必选参数 ： ： 动态 id，可通过 ， 接口获取threadId/event/user/event
...
关注/取消关注用户
说明 ： 登录后调用此接口 ， 传入用户 ID， 和作 t，可关注/取消关注用户
...
可选参数 ： ： type=1 时只返回 weekData， type=0 时返回 allDatatype
...
心动模式/智能播放
说明 ： 登录后调用此接口 ， 可获取心动模式/智能播放列表 必选参数 ： ： 歌曲 IDid

pid： 歌单 ID

可选参数 ： ： 要开始播放的歌曲的 idsid
...
offset: 偏移数量，用于分页 , 如 : 如 :(页数 -1）*30， 其中 30 为 limit 的值 ， 默认为 0 ： 按首字母索引查找参数，如 返回内容将以 name 字段开头为 b 或者拼音开头为 b 为顺序排列， 热门传-1，#传 0initial/artist/list?type=1&area=96&initial=b
...
可选参数 : : 可选值为 'new' 和 'hot', 分别对应最新和最热 , 默认为 'hot'order
...
可选参数 : : tag, 比如 " 华语 "、" 古风 " 、" 欧美 "、" 流行 ", 默认为 "全部",可从精品歌单标签列表接口获取(cat/playlist/highquality/tags)

limit: 取出歌单数量 , 默认为 50

before: 分页参数,取上一页最后一个歌单的 获取下一页数据updateTime
...
相关歌单
说明: 请替换为相关歌单推荐接口; 本接口通过html抓取内容, 现已无法抓取歌单
...
获取歌单详情
说明 : 歌单能看到歌单名字, 但看不到具体歌单内容 , 调用此接口 , 传入歌单 id, 可 以获取对应歌单内的所有的音乐(未登录状态只能获取不完整的歌单,登录后是完整的)，但是返回的 trackIds 是完整的，tracks 则是不完整的，可拿全部 trackIds 请求一次 接口获取所有歌曲的详情 (https://github.com/Binaryify/NeteaseCloudMusicApi/issues/452song/detail)

必选参数 : : 歌单 idid

可选参数 : : 歌单最近的 s 个收藏者,默认为 8s
...
可选参数 : : 码率,默认设置了 999000 即最大码率,如果要 320k 则可设置为 320000,其他类推br
...
必选参数 : : 音乐 id : 播放音质等级, 分为 => , => , =>, =>, =>, => , => , => , idlevelstandard标准higher较高exhigh极高lossless无损hiresHi-Resjyeffect高清环绕声sky沉浸环绕声dolby杜比全景声jymaster => 超清母带

接口地址 : /song/url/v1

调用例子 : /song/url/v1?id=33894312&level=exhigh /song/url/v1?id=405998841,33894312&level=lossless

说明：音质需要设备支持，不同的设备可能会返回不同码率的url。cookie需要传入保证返回正常码率的url。杜比全景声os=pc

音乐是否可用
说明: 调用此接口,传入歌曲 id, 可获取音乐是否可用,返回 或者 { success: true, message: 'ok' }{ success: false, message: '亲爱的,暂无版权' }
...
搜索
说明 : 调用此接口 , 传入搜索关键词可以搜索该音乐 / 专辑 / 歌手 / 歌单 / 用户 , 关键词可以多个 , 以空格隔开 , 如 " 周杰伦 搁浅 "( 不需要登录 ), 可通过 接口传入歌曲 id 获取具体的播放链接/song/url

必选参数 : : 关键词keywords

可选参数 : : 返回数量 , 默认为 30 : 偏移数量，用于分页 , 如 : 如 :( 页数 -1)*30, 其中 30 为 limit 的值 , 默认为 0limitoffset

type: 搜索类型；默认为 1 即单曲 , 取值意义 : 1: 单曲, 10: 专辑, 100: 歌手, 1000: 歌单, 1002: 用户, 1004: MV, 1006: 歌词, 1009: 电台, 1014: 视频, 1018:综合, 2000:声音(搜索声音返回字段格式会不一样)

接口地址 : 或者 (更全)/search/cloudsearch
...
pid: 歌单 id : 歌曲 id,可多个,用逗号隔开tracks

接口地址 : /playlist/tracks

调用例子 : ( 对应把歌曲添加到 ' 我 ' 的歌单 , 测试的时候请把这里的 pid 换成你自己的, id 和 tracks 不对可能会报 502 错误)/playlist/tracks?op=add&pid=24381616&tracks=347231
...
获取逐字歌词
说明 : 此接口的 字段即为逐字歌词 (可能有歌曲不包含逐字歌词)yrc
...
当逐字歌词适用时，的字段包括形式如下的内容yrclyric

（可能存在）JSON 歌曲元数据该字段不一定出现；可能出现的数据意义有：
...
before: 分页参数,取上一页最后一项的 获取下一页数据(获取超过 5000 条评论的时候需要用到)time

接口地址 : /comment/music

调用例子 ： 对应晴天评论/comment/music?id=186016&limit=1

楼层评论
说明 ： 调用此接口 ， 传入资源 parentCommentId 和资源类型 类型 和资源 id 参数， 可获得该资源的歌曲楼层评论

必选参数 ：
： 楼层评论 IDparentCommentId
...
time: 分页参数,取上一页最后一项的 获取下一页数据time
...
7: 电台
可选参数 ：
：分页参数，第 N 页，默认为 1pageNo

pageSize:分页参数,每页多少条数据,默认 20

sortType: 排序方式, 1:按推荐排序, 2:按热度排序, 3:按时间排序

cursor： 当为 3 时且页数不是第一页时需传入，值为上一条数据的 timesortType
...
调用例子 ： 对应给 https://music.163.com/#/song?id=29178366 最热门的评论点赞/comment/like?id=29178366&cid=12840183&t=1&type=0

注意： 动态点赞不需要传入 id 参数，需要传入动态的 参数,如：， 可通过 ， 接口获取threadId/comment/like?type=6&cid=1419532712&threadId=A_EV_2_6559519868_32953014&t=0threadId/event/user/event
...
调用例子 ： （往广岛之恋 mv 发送评论： test）/comment?t=1&type=1&id=5436712&content=test

注意：如给动态发送评论，则不需要传 id，需要传动态的 ,如：threadId/comment?t=1&type=6&threadId=A_EV_2_6559519868_32953014&content=test
...
```  
id:对应资源 id :内容 id,可通过 等接口获取content/comment/mv

调用例子 : (在广岛之恋 mv 删除评论)/comment?t=0&type=1&id=5436712&commentId=1535550516319

注意：如给动态删除评论，则不需要传 id，需要传动态的 ,如：threadId/comment?t=0&type=6&threadId=A_EV_2_6559519868_32953014&commentId=1419516382
...
注意：如给动态点赞，不需要传入 id，需要传入 ,可通过 , 接口获取，如： threadIdevent/user/event/resource/like?t=1&type=6&threadId=A_EV_2_6559519868_32953014
...
调用例子 : ( 周杰伦 )/album/sublist

获取歌手单曲
说明 : 调用此接口 , 传入歌手 id, 可获得歌手部分信息和热门歌曲

必选参数 : : 歌手 id, 可由搜索接口获得id
...
获取歌手 mv
说明 : 调用此接口 , 传入歌手 id, 可获得歌手 mv 信息 , 具体 mv 播放地址可调 用传入此接口获得的 mvid 来拿到 , 如 : ,/mv/artist/mv?id=6452/mv?mvid=5461064
...
可选参数 : : 取出数量 , 默认为 30limit

offset: 偏移数量 , 用于分页 , 如 :( 页数 -1)*30, 其中 30 为 limit 的值 , 默认 为 0

接口地址 : /artist/album

调用例子 : ( 周杰伦 )/artist/album?id=6452&limit=5
...
调用例子 : (Billie Eilish)/artist/detail?id=11972054
...
调用例子 : ( 对应和周杰伦相似歌手 )/simi/artist?id=6452
...
调用例子 : ( 对应 ' 光辉岁月 ' 相似歌单 )/simi/playlist?id=347230

相似 mv
说明 : 调用此接口 , 传入 可获取相似 mvmvid
...
调用例子 : ( 对应 ' 光辉岁月 ' 相似歌曲 )/simi/song?id=347230
...
调用例子 : ( 对应 ' 光辉岁月 ' 相似歌曲 )/simi/user?id=347230
...
必选参数 : : 日期,通过历史日推可用日期列表接口获取,不能任意日期date
...
可选参数 : : 签到类型 , 默认 0, 其中 0 为安卓端签到 ,1 为 web/PC 签到type
...
可选参数 : : 布尔值 , 默认为 true 即喜欢 , 若传 false, 则取消喜欢like
...
新碟上架
说明 : 调用此接口 , 可获取新碟上架列表 , 如需具体音乐信息需要调用获取专辑列表接 口 , 然后传入 id, 如 /album/album?id=32311
...
必选参数 : : 歌曲 id, : 歌单或专辑 ididsourceid

可选参数 : : 歌曲播放时间,单位为秒time
...
可选参数 :
: 地区,可选值为全部,内地,港台,欧美,日本,韩国,不填则为全部 : 类型,可选值为全部,官方版,原生,现场版,网易出品,不填则为全部areatype
...
可选参数 : : 地区,可选值为全部,内地,港台,欧美,日本,韩国,不填则为全部area
...
可选参数 : : 取出数量 , 默认为 30 (不支持 offset)limit
...
可选参数 : : 取出数量 , 默认为 10 (不支持 offset)limit
...
可选参数 : : 分辨率,默认 1080,可从 接口获取分辨率列表r/mv/detail
...
获取视频标签/分类下的视频
说明 ： 调用此接口 ， 传入标签/分类，可获取到相关的视频，分页参数只能传入 offsetid

必选参数 ： ： videoGroup 的 idid
...
获取视频点赞转发评论数数据
说明 : 调用此接口 , 传入 vid ( 视频 id ) , 可获取对应视频点赞转发评论数数据 必选参数 : : 视频 idvid
...
云盘
说明 : 登录后调用此接口 , 可获取云盘数据 , 获取的数据没有对应 url, 需要再调用一 次 获取 url/song/url
...
云盘上传
说明 : 登录后调用此接口,使用上传 mp3 formData(name 为'songFile'),可上传歌曲到云盘'Content-Type': 'multipart/form-data'
...
电台订阅者列表
说明 : 调用此接口,可获取电台订阅者列表 必选参数 : : 电台 idid

可选参数 : : 分页参数,默认-1,传入上一次返回结果的 time,将会返回下一页的数据time
...
cateId: 类别 id,可通过 接口获取/dj/category/recommend

接口地址 : /dj/radio/hot

调用例子 : (创作|翻唱) (3D|电子)/dj/radio/hot?cateId=2001/dj/radio/hot?cateId=10002
...
必选参数 : : 电台类型 , 数字 , 可通过获取 , 对应关系为 id 对应 此接口的 type, name 对应类型type/dj/catelist

接口地址 : /dj/recommend/type

调用例子 : (明星做主播) (创作|翻唱)/dj/recommend/type?type=1/dj/recommend/type?type=2001

电台 - 订阅
说明 : 登录后调用此接口 , 传入, 可订阅 dj,dj 的 可通过搜索指定 type='1009' 获取其 id, 如ridrid/search?keywords= 代码时间 &type=1009

必选参数 : : 电台 的 idrid

接口地址 : /dj/sub

调用例子 : ( 对应关注 ' 代码时间 ') ( 对应取消关注 ' 代码时间 ')/dj/sub?rid=336355127&t=1/dj/sub?rid=336355127&t=0
...
电台 - 付费精选
说明 : 可以获取付费精选的电台列表 , 传入 和 可以进行分页limitoffset
...
电台 - 详情
说明 : 登录后调用此接口 , 传入, 可获得对应电台的详情介绍rid

必选参数 : : 电台 的 idrid

接口地址 : /dj/detail

调用例子 : ( 对应 ' 代码时间 ' 的详情介绍 )/dj/detail?rid=336355127

电台 - 节目
说明 : 登录后调用此接口 , 传入, 可查看对应电台的电台节目以及对应的 id, 需要 注意的是这个接口返回的 mp3Url 已经无效 , 都为 null, 但是通过调用 这 个接口 , 传入节目 mainTrackId 仍然能获取到节目音频 , 如 获取代 码时间的一个节目的音频rid/song/url/song/url?id=478446370
...
asc : 排序方式,默认为 (新 => 老 ) 设置 可改为 老 => 新falsetrue

接口地址 : /dj/program

调用例子 : ( 对应 ' 代码时间 ' 的节目列表 )/dj/program?rid=336355127&limit=40
...
before : 分页参数,取上一页最后一项的 获取下一页数据time

接口地址 : /msg/private/history

调用例子 : (云音乐小秘书)/msg/private/history?uid=9003

通知 - 评论
说明 : 登录后调用此接口 ,可获取评论

必选参数 : : 用户 的 id，只能和登录账号的 id 一致uid
...
lasttime : 返回数据的 ,默认-1,传入上一次返回结果的 time,将会返回下一页的数据time
...
调用例子 : 使用 GET 方式: 使用 POST 方式传入参数:/batch?/api/v2/banner/get={"clientType":"pc"}{ "/api/v2/banner/get": {"clientType":"pc"} }
...
将 mlog id 转为视频 id
说明 : 调用此接口 , 传入 mlog id, 可获取 video id，然后通过 获取播放地址video/url
...
必选参数 : : 通过获取到的ids/vip/tasksunGetIds
...
歌手粉丝
说明 : 调用此接口 , 传入歌手 id, 可获取歌手粉丝 必选参数 : : 歌手 idid
...
音乐人任务
说明 : 音乐人登录后调用此接口 , 可获取音乐人任务。返回的数据中字段为任务状态，0 表示任务未开始，10 表示任务正在进行中，20 表示任务完成，但未领取云豆，100 表示任务完成，并且已经领取了相应的云豆(貌似只能获取到做过的任务了)status
...
音乐人任务(新)
说明 : 音乐人登录后调用此接口 , 可获取音乐人任务。返回的数据中字段为任务状态，0 表示任务未开始，10 表示任务正在进行中，20 表示任务完成，但未领取云豆，100 表示任务完成，并且已经领取了相应的云豆status
...
必选参数 : : 任务 id，通过获取到的即为任务 idid/musician/tasksuserMissionId

period : 通过获取/musician/tasks
...
音乐人签到
说明: 音乐人登录后调用此接口，可以完成“登录音乐人中心”任务，然后通过接口可以领取相应的云豆。/musician/cloudbean/obtain
...
可选参数 : : 如果定义，此 mvid 对应的 MV 将会作为第一个返回。 : 取出的 Mlog 数量, 不包含第一个 mvidmvidlimit
...
获取客户端歌曲下载 url
说明 : 使用 接口获取的是歌曲试听 url, 但存在部分歌曲在非 VIP 账号上可以下载无损音质而不能试听无损音质, 使用此接口可使非 VIP 账号获取这些歌曲的无损音频/song/url

必选参数 : : 音乐 id (仅支持单首歌曲)id
...
可选参数 : : 模块 id，默认为 '1207signin-1207signin'moduleId
...
可选参数 : : 开始时间 : 结束时间 : 返回数量 , 默认为 60startTimeendTimelimit

接口地址 : /vip/timemachine

调用例子 : （2021年12月） (2021年)/vip/timemachine/vip/timemachine?startTime=1638288000000&endTime=1640966399999&limit=10/vip/timemachine?startTime=1609430400&endTime=1640966399999&limit=60
...
必选参数： : 播客idvoiceListId

返回结果的参数对应:displayStatus

AUDITING 审核中
ONLY_SELF_SEE 仅自己可见
ONLINE 已发布
可选参数： : 取出歌单数量 , 默认为 200limit
...
播客上传声音
说明: 可以上传声音到播客,例子在 访问地址: /voice_upload.html/public/voice_upload.html

接口地址: /voice/upload

必选参数： : 播客 id voiceListId
...
验证接口-二维码检测
说明: 使用此接口,传入接口返回的字符串,可检测二维码扫描状态/verify/getQrqr

接口地址: /verify/qrcodestatus

必选参数：

qr: 接口返回的字符串/verify/getQrqr
...
听歌识曲
说明: 使用此接口,上传音频文件或者麦克风采集声音可识别对应歌曲信息,具体调用例子参考 (项目文件:/audio_match_demo/index.htmlpublic/audio_match_demo/index.html)
...
根据nickname获取userid
说明: 使用此接口,传入用户昵称,可获取对应的用户id,支持批量获取,多个昵称用隔开分号(;)
...
歌曲音质详情
说明: 调用此接口获取歌曲各个音质的文件信息，与 接口相比，多出 、、等音质的信息获取歌曲详情高清环绕声沉浸环绕声超清母带
...
为保证成功,请使用 接口获取各文件属性获取音乐url

其中比特率要进行以下转换bitrate

bitrate = Math.floor(br / 1000)
导入后的文件名后缀均为 。但用 获取到的文件格式仍然是正确的。.mp3获取音乐url

获取客户端歌曲下载链接 - 新版
说明 : 使用 接口获取的是歌曲试听 url, 非 VIP 账号最高只能获取 音质，但免费类型的歌曲()使用本接口可最高获取音质的url。/song/url/v1极高fee == 0Hi-Res
...
用户状态 - 编辑
说明 : 登录后调用此接口, 编辑当前用户状态， 所需参数可在接口获取/user/social/status/support
...
importStarPlaylist : 是否导入, 此项为true则不生成新的歌单 : 生成的歌单名, 仅文字导入和链接导入支持, 默认为我喜欢的音乐playlistName'导入音乐 '.concat(new Date().toLocaleString())
